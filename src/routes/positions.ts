  ...
  try {
    ...
    try {
      ...
      await createAuditLog({
        userId,
        action: 'ORDER_PLACE',
        summary: `Order placed: ${side} ${calculatedQty} ${symbol}`,
        payload: { symbol, side, qty: calculatedQty, price, type, orderId: result.id }
      });
      ...
      return res.json({ orderId: result.id, qty: calculatedQty });
    } finally {
